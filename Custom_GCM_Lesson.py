...
import config as cf
from Toy_GCM import perform_toy_GCM
...
def get_steps(canvas, root, info_lbl, next_btn, prev_btn, values):
    # Defining Constants
    W = 750
    H = 600
    plaintext_blocks = values[:3]
    key = values[3]
    iv = values[4]
    IVs, KS, full_CT, Y_sequence, H_value = perform_toy_GCM(plaintext_blocks, key, iv)
    c = get_dark_mode()
    ...
    def step_0():
        ...
        canvas.create_text(W/2, H/2,
            text="GCM (Galois/Counter Mode) is a Mode of Operation that builds upon CTR.\n"
                 ...
                 "For this example you selected the following values:\n"
                 f"Plaintext: {','.join(plaintext_blocks)}\n"
                 f"Key: {key}\n"
                 f"IV: {iv}",
            font=cf.Fonts["body"],
            fill=c["text"],
    ...
    def step_2():
        ...
        for i in range(3):
            x_center = column_width * i + column_width/2
            block_tag = "Key" + str(i)
            draw_block(canvas, root, x_center, 300, key, cf.Block_colours["key"], 1,  block_tag)
        ...
        root.after(2000, lambda: (canvas.delete("arrow1"), canvas.delete("IV0"), canvas.itemconfigure("Key0_text", text=KS[0]), canvas.itemconfigure("Key0_rect", outline=cf.Block_colours["keystream"])))
        root.after(2100, lambda: move_block_down(canvas, root, "IV1", 300))
        root.after(4100, lambda: (canvas.delete("arrow2"), canvas.delete("IV1"),canvas.itemconfigure("Key1_text", text=KS[1]), canvas.itemconfigure("Key1_rect", outline=cf.Block_colours["keystream"])))
        root.after(4200, lambda: move_block_down(canvas, root, "IV2", 300))
        root.after(6200, lambda: (canvas.delete("arrow3"), canvas.delete("IV2"),canvas.itemconfigure("Key2_text", text=KS[2]),canvas.itemconfigure("Key2_rect", outline=cf.Block_colours["keystream"])))
    ...
    def step_3():
        ...
        KS1 = []
        CT1 = []
        for char in plaintext_blocks[0]:
            PT1.append(char)
        for char in KS[0]:
            KS1.append(char)
        for char in full_CT[0]:
            CT1.append(char)
    ...
    def step_6():
        ...
        info_lbl.config(text="")
        canvas.create_text(W / 2, H / 2,
                           text="In order to create this authentication tag, we start with a value Y which starts 0000\n"
                                "This value will constantly change as the tag is being calculated\n"
                                "The final value of Y is the authentication tag.\n"
    ...
    def step_7():
        ...
        root.after(6000, lambda: draw_block(canvas, root, 375, 500, H_value, cf.Block_colours["h"], 1, "H"))
        ...
        for i in range(2):
            ...
            root.after(delay + 5000, lambda: (draw_block(canvas, root, 375, 500, H_value, cf.Block_colours["h"], 1, "H"), canvas.itemconfig("X_text", state="normal")))
    ...
    def step_8():
        ...
        canvas.create_text(W / 2, H / 2,
                           text="This gives us the following values which will then be sent to the recipient\n"
                                f"Ciphertext: {','.join(full_CT)}\n"
                                f"IV: {iv}\n"
                                f"Authentication Tag: {Y_sequence[-1]}\n",
                           font=cf.Fonts["body"],
                           fill=c["text"],
                           anchor="center")
